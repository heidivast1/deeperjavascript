 ...
 */
 ...
 createSpaces(){
     ...
     for (let x = 0; x < this.columns; x++){
       const col = []; /*will hold the Space objects for each column
       ...
       for (let y = 0; y < this.rows; y++){ /*new Space objects are created*/
         ...
         col.push(space); /*push newly created space to the column array, on each iteration through loop another apce is added to the column*/
      }

      spaces.push(col); /*push entire column into Spcaes array, then the x index is increased by 1 and the process continues*/
    }
     return spaces;
   }
